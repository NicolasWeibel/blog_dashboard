...
import { get_author_blog } from "redux/actions/blog/blog";
...
function EditPost({
  ...
  const onSubmit = (e) => {
    ...
    sendFormData.append("new_slug", new_slug);
    sendFormData.append("description", description);
    sendFormData.append("category", category);
    sendFormData.append("time_read", time_read);

    if (thumbnail) {
      sendFormData.append("thumbnail", thumbnail, thumbnail.name);
    } else {
      sendFormData.append("thumbnail", "");
    }

    if (content) {
      sendFormData.append("content", content);
    } else {
      sendFormData.append("content", "");
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (res.status === 200) {
          if (new_slug !== "") {
            await get_author_blog(new_slug);
            navigate(-1);
          } else {
            await get_author_blog(slug);
          }
              ...
              <div className="ml-4 mt-4 flex-shrink-0">
                ...
                {post.status === "published" ? (
                  <Link
                    ...
                    target="_blank"
                    className="relative mx-1 inline-flex items-center rounded-md border border-transparent bg-gray-400 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2"
                  >
                    View Post
                  </Link>
                ) : (
                  <Link
                    to="#"
                    className="cursor-not-allowed relative mx-1 inline-flex items-center rounded-md border border-transparent bg-gray-400 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-gray-300 focus:ring-offset-2"
                  >
                    View Post
                  </Link>
                )}
